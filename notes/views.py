...
def topics_views(request):
    ...
    return render(request, 'notes/topics.html', locals())
...
def topic_views(request, topic_id):
    ...
    return render(request, 'notes/topic.html', locals())
...
def new_topic_views(request):
    ...
    return render(request, 'notes/new_topic.html', locals())
...
def edit_topic_views(request, topic_id):
    ...
    if request.method == 'GET':
        ...
        if form.is_valid():
            ...
            form.save()
            return HttpResponseRedirect(reverse('learning_logs:topics'))
    return render(request, 'notes/edit_topic.html', locals())
...
def delete_topic_views(request, topic_id):
    ...
    return render(request, 'notes/delete_topic.html', locals())
...
def new_entry_views(request, topic_id):
    ...
    return render(request, 'notes/new_entry.html', locals())
...
def edit_entry_views(request, entry_id):
    ...
    return render(request, 'notes/edit_entry.html', locals())
...
def delete_entry_views(request, entry_id):
    ...
    # 删除笔记
    entry.delete()
    return render(request, 'notes/delete_entry.html', locals())
...
def query_entry_views(request):
    ...
    if request.method == 'GET':
        ...
        if form.is_valid():
            ...
            # topic表里 找text 得到id 再在entry表中找topic_id=id
            topics = Topic.objects.filter(text__contains=text['text'])

            for i in topics:
                item = Entry.objects.filter(topic_id=i.id)
                entries = entries | item

            for i in entries:
                topic = Topic.objects.get(id=i.topic_id)
                i.topic_text=topic.text

            return render(request, 'notes/query_entry_result.html', locals())
    return render(request, 'notes/query_entry.html', locals())
...
def query_book_views(request):
    ...
    if request.method == 'GET':
        ...
        if form.is_valid():
            ...
            driver.get(url)
            # 找到元素，输入信息，点击搜索按钮
            elements = driver.find_elements(By.CLASS_NAME, 'title-text')
            elements[1].click()

            # 图书信息、作者信息
            pattern1 = re.compile('<div class="intro">([\s\S]*?)</div>')
            data11 = re.findall(pattern1, driver.page_source)
            # 清理多余标签
            data1 = []
            for intro in data11:
                soup = BeautifulSoup(intro, 'html.parser')
                text = soup.get_text()
                data1.append(text)

            # 图书豆瓣评分信息
            pattern4 = re.compile('<span class="rating_per">([\s\S]*?)</span>')
            data4 = re.findall(pattern4, driver.page_source)

            # 图书商店价格统计
            pattern4 = re.compile('<span class="rating_per">([\s\S]*?)</span>')
            data4 = re.findall(pattern4, driver.page_source)
            # 点击更多短评
            driver.find_element(By.XPATH, '//*[@id="comments-section"]/p/a').click()

            # 图书评论20条
            pattern2 = re.compile('<p class="comment-content">([\s\S]*?)</p>')
            data22 = re.findall(pattern2, driver.page_source)
            # 清理多余标签
            data2 = []
            for comment in data22:
            ...
            chart.render_to_file(
                'notes/templates/notes/bookZan.svg')

            # 图2
            my_style2 = pygal.style.LightColorizedStyle
            pie_chart = pygal.Pie()
            pie_chart.title = '豆瓣评分分布图'
            idx = 5
            for a in data4:
                pie_chart.add(str(idx) + "星评价", float(a[:-1]), value_formatter=lambda x: "{x}%")
                idx -= 1
            pie_chart.render_to_file('notes/templates/notes/comment.svg')
            ...
            # 重定向页面
            return render(request, 'notes/query_book_result.html', locals())
    return render(request, 'notes/query_book.html', locals())


@login_required
def query_book_svg(request):
    """图书评论赞可视化文件"""
    return render(request, 'notes/bookZan.svg', {})


@login_required
def behavior_comment_svg(request):
    return render(request, 'notes/comment.svg', {})
...
def behavior_analysis_views(request):
    ...
    chart.add('', num)
    chart.render_to_file('notes/templates/notes/analysis.svg')

    # 数据可视化散点图
    my_config2 = pygal.Config()
    my_config2.width = 1000
    my_config2.height = 590
    chartSD = pygal.XY(my_config2)  # 创建散点图对象
    chartSD.title = '历史搜索散点图'

    chartSD.x_labels = None

    # 添加散点数据
    for i in range(len(text)):
        chartSD.add(text[i], [(i, num[i])])

    chartSD.render_to_file('notes/templates/notes/analysisSD.svg')  # 保存为SVG文件
    ...
    return render(request, 'notes/behavior_analysis.html', locals())


@login_required
def behavior_analysis_svg(request):
    return render(request, 'notes/analysis.svg', {})


@login_required
def behavior_analysisSD_svg(request):
    return render(request, 'notes/analysisSD.svg', {})
